extern crate safe_fuse;

use safe_fuse::config;
use std::env;
use std::process;
use std::fs;
...
fn usage() {
  eprintln!("USAGE:");
  eprintln!("  safe-fuse init <local dir> <remote source> <max local size in MB>");
  eprintln!("  safe-fuse clone <local dir> <remote source> <max local size in MB>");
  eprintln!("  safe-fuse mount <local dir> <mount dir>");
  process::exit(2);
}
...
fn init(args: &[String], fetch: bool) {
  ...
  source.push("data");
  if fetch {
    match safe_fuse::clone(&source, &conf) {
      Ok(_) => {},
      Err(e) => eprintln!("CLONE ERROR: {}", e),
    }
  } else {
    match safe_fuse::init(&source, &conf) {
      Ok(_) => {},
...
fn mount(args: &[String]) {
  ...
  match safe_fuse::run(&source, &mount, &conf) {
    Ok(_) => {},
...
fn printlog(args: &[String]) {
  ...
  match safe_fuse::printlog(&source, &conf) {
    Ok(_) => {},
